...
#ifdef _WINDOWS
	static char	DEFAULT_CONFIG_FILE[]	= "C:\\whatap_agent.conf";
#else
	static char	DEFAULT_CONFIG_FILE[]	= SYSCONFDIR "/whatap_agent.conf";
#endif
...
const char	*help_message[] = {
	...
	"Functions:",
	"",
	"  -i --install          Install Whatap agent as service",
	"  -d --uninstall        Uninstall Whatap agent from service",

	"  -s --start            Start Whatap agent service",
	"  -x --stop             Stop Whatap agent service",
...
int	MAIN_ZABBIX_ENTRY()
{
	...
	zabbix_log(LOG_LEVEL_INFORMATION, "Starting Whatap Agent [%s]. Whatap %s (revision %s).",
...
void	zbx_free_service_resources(void)
{
	...
	zabbix_log(LOG_LEVEL_INFORMATION, "Whatap Agent stopped. Whatap %s (revision %s).",
