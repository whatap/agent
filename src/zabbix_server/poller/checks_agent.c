...
int	get_value_agent(DC_ITEM *item, AGENT_RESULT *result)
{
	...
	if (SUCCEED == ret)
	{
		...
		{
			zbx_snprintf(buffer, sizeof(buffer), "Not supported by Whatap Agent");
			SET_MSG_RESULT(result, strdup(buffer));
			ret = NOTSUPPORTED;
		}
		else if (0 == strcmp(buf, ZBX_ERROR))
		{
			zbx_snprintf(buffer, sizeof(buffer), "Whatap Agent non-critical error");
			SET_MSG_RESULT(result, strdup(buffer));
			ret = AGENT_ERROR;
		}
		else if (0 == received_len)
		{
			zbx_snprintf(buffer, sizeof(buffer), "Received empty response from Whatap Agent at [%s]."
