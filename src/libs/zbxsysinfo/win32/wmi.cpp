...
unsigned long WHATAP_WMI_GETI(const char *wmi_namespace, const char* wmi_query)
{
	IWbemClassObject	*pclsObj = 0;
	ULONG			uReturn = 0;
	ULONG			uResult = 0;
	...
	while( SUCCEEDED(pclsObj->Next(0, NULL, &vtProp, 0, 0))){
		if (0 != (vtProp.vt & VT_ARRAY))
		{
			zabbix_log(LOG_LEVEL_DEBUG, "cannot convert WMI array result");
			goto out;
		...
		switch(vtProp.vt)
		{
			...
			case VT_INT:
			case VT_UINT:
				hres = VariantChangeType(&vtProp, &vtProp, 0, VT_UI8);
			...
			default:
				...
				uResult += (unsigned)atol((char*)_bstr_t(vtProp.bstrVal));
				break;
		}
		VariantClear(&vtProp);
	}
