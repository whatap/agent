...
void	get_active__net_if_list(int* net_if_flag)
{
	DWORD		dwRetVal;
	...
	do {
        pAddresses = (IP_ADAPTER_ADDRESSES *) zbx_malloc(pAddresses, outBufLen);
        if (pAddresses == NULL) {
            printf
	...
	if (dwRetVal == NO_ERROR) {
        ...
        while (pCurrAddresses) {
			if( pCurrAddresses->IfType== 6&& pCurrAddresses->OperStatus== 1){
				zabbix_log(LOG_LEVEL_DEBUG, "active: %d", pCurrAddresses->IfIndex);
				if (pCurrAddresses->IfIndex > 0 && pCurrAddresses->IfIndex < 128){
					*(net_if_flag+pCurrAddresses->IfIndex)=1;
				}
			}else{
				zabbix_log(LOG_LEVEL_DEBUG, "inactive: %d", pCurrAddresses->IfIndex);
				if (pCurrAddresses->IfIndex > 0 && pCurrAddresses->IfIndex < 128){
					*(net_if_flag+pCurrAddresses->IfIndex)=0;
				}
			}
...
int	NET_IF_DISCOVERY(AGENT_REQUEST *request, AGENT_RESULT *result)
{
	...
	struct zbx_json	j;
	LPSTR		utf8_descr;
	int	net_if_flag[NET_IF_FLAG_SIZE];
	
	memset(net_if_flag, 0 , sizeof(int)*NET_IF_FLAG_SIZE);
	...
	get_active__net_if_list(net_if_flag);
	...
	for (i = 0; i < pIfTable->dwNumEntries; i++)
	{
		...
		if (pIfRow.dwIndex >= 0 && pIfRow.dwIndex < NET_IF_FLAG_SIZE && net_if_flag[pIfRow.dwIndex] == 0){
			continue;
		}
